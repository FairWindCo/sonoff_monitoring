...
def list_device_status_ajax(request):
    devices, *_ = paginate(request, SonoffDevices.objects.all().order_by('pk'))
    status = []
    for device in devices:
            status.append({
                ...
                'last': formats.date_format(timezone.localtime(device.date_of_modify),'d M Y H:i:s') if device.is_online else formats.date_format(timezone.localtime(device.last_off_line),'d M Y H:i:s'),
...
def group_status_ajax(request, group_id=None):
    ...
    devices, *_ = paginate(request, device_group.devices.all().order_by('pk'))
    status = []
    for device in devices:
            status.append({
                'id': device.id,
...
def device_graph(request, device_id):
    ...
    if device:
            ...
            before_date = curent_date - timedelta(days=1)

            from_date = get_request_param(request, 'from_date', before_date.strftime('%m/%d/%Y %I:%M %p'))
            end_date = get_request_param(request, 'end_date', curent_date.strftime('%m/%d/%Y %I:%M %p'))
            ...
            device_values = [(val.humidity, val.temperature, val.date_of_modify)
                             for val in device.values.filter(date_of_modify__range=(f_date, t_date)).order_by('date_of_modify')]
...
def list_device_offline(request):
    per_page_values = [1, 10, 25, 50, 100]
    return request_paginator_form(request, 'devices/list_offline_devices.html', SonoffDevices.objects.filter(is_online=False).order_by('pk'),
                                  result_field_name='devices',
                                  per_page_values=per_page_values,
                                  search_attributes={'id': 'device_id__icontains',
                                                     'name': 'name__icontains'})
